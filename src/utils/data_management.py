...
def validate_image(config_file: dict) -> None:
    PARENT_DIR = os.path.join(config_file["data"]["data_dir"],
        config_file["data"]["unzip_data_dir"],
        config_file["data"]["parent_data_dir"])
    BAD_DATA_DIR = os.path.join(
        config_file["data"]["data_dir"],
        config_file["data"]["unzip_data_dir"],
        config_file["data"]["bad_data_dir"])
    ...
    for dirs in os.listdir(PARENT_DIR):
        ...
        for imgs in os.listdir(full_path_data_dir):
            ...
            try:
                ...
                img.verify()
                if len(img.getbands()) !=3 or imghdr.what(path_to_img) not in ['jpeg','jpg','png']:
